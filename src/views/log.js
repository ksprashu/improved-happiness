...
import { addLog, updateLog, deleteLog } from "../state.js";
...
export function renderLog(state) {
  ...
  main.appendChild(createLogForm());
  main.appendChild(renderLogList(state));
...
function createHeader() {
  ...
  header.innerHTML = `<h1>Training log</h1><p>Capture volume, breath, and insights.</p>`;
  return header;
}
...
function createLogForm() {
  ...
  form.appendChild(createInput("number", "duration", "Duration (minutes)", 30, { min: 5, max: 120 }));
  ...
  exerciseList.className = "form-field";
  exerciseList.innerHTML = `<label>Exercises performed</label>`;
  const exerciseContainer = document.createElement("div");
  exerciseContainer.dataset.role = "exercise-container";
  exerciseContainer.style.display = "flex";
  exerciseContainer.style.flexDirection = "column";
  exerciseContainer.style.gap = "0.5rem";

  function addExerciseRow(name = "", prescription = "") {
    ...
    prescriptionInput.placeholder = "Sets × reps or time";
    ...
    row.appendChild(prescriptionInput);
  ...
  form.appendChild(exerciseList);

  form.appendChild(createInput("text", "loadSummary", "Load / density notes", ""));
  ...
  rpeValue.style.marginTop = "0.4rem";
  rpeValue.style.color = "var(--text-muted)";
  ...
  form.appendChild(createInput("number", "breathMinutes", "Breathwork completed (minutes)", 3, { min: 0, max: 20 }));
  ...
  form.addEventListener("submit", (event) => {
    ...
    const exercises = Array.from(exerciseContainer.children).map((row) => {
      const inputs = row.querySelectorAll("input");
      const name = inputs[0].value.trim();
      const prescription = inputs[1].value.trim();
      if (!name) return null;
      return { name, reps: prescription };
    }).filter(Boolean);
  ...
  state.logs.forEach((log) => {
    ...
    if (log.exercises?.length) {
      ...
      log.exercises.forEach((exercise) => {
        ...
        li.textContent = `${exercise.name} — ${exercise.reps}`;
    ...
    load.textContent = log.loadSummary ? `Load: ${log.loadSummary}` : "";
    if (load.textContent) card.appendChild(load);
